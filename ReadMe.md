...
This is a sample code for Melissa's Property API.
...
Inside the folder will be the build under `property` as well as an all-in-one `property.zip`.
...
To run the example, navigate to `./target/property/bin/` and run `property.bat`.
